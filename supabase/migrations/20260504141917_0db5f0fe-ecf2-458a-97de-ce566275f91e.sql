-- ============================================================
-- RPC folha_pagamento — Motor financeiro determinístico
-- Versão da regra: v1
-- ============================================================
...
BEGIN
  ...
  IF _tenant_id IS NULL THEN
    RAISE EXCEPTION 'Não autenticado ou sem tenant';
  END IF;
  ...
  ajustes AS (
    ...
    WHERE a.tenant_id = _tenant_id
      ...
      AND a.periodo_fim >= _data_inicio
      AND a.periodo_inicio <= _data_fim
  ...
  consolidado AS (
    ...
    WHERE c.tenant_id = _tenant_id
      ...
      AND (
        EXISTS (SELECT 1 FROM presencas p WHERE p.colaborador_id = c.id)
  ...
  linhas AS (
    SELECT
      jsonb_build_object(
        ...
        'nome', co.nome,
        'qtd_presenca', co.qtd_presenca,
        'valor_presenca', co.valor_presenca,
        'qtd_ajuste', co.qtd_ajuste,
        'valor_ajuste', co.valor_ajuste,
        ...
        'breakdown', jsonb_build_object(
          'presencas', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
              'data', p.data,
              'tipo', p.tipo,
              'fracao', p.fracao_diaria,
              'valor_diaria', p.valor_diaria_usado,
              'valor', p.valor
            ) ORDER BY p.data, p.id)
            FROM presencas p WHERE p.colaborador_id = co.colaborador_id
          ), '[]'::jsonb),
          'ajustes', COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
              ...
              'fim', a.periodo_fim,
              'tipo', a.tipo,
              'quantidade', a.quantidade_diarias,
              'valor_diaria', a.valor_diaria,
              'valor', a.valor
            ) ORDER BY a.periodo_inicio, a.id)
            FROM ajustes a WHERE a.colaborador_id = co.colaborador_id
          ), '[]'::jsonb)
        )
      ) AS linha,
      co.nome AS sort_nome,
      co.colaborador_id AS sort_id
    FROM consolidado co
  ),
  ordered AS (
    SELECT COALESCE(jsonb_agg(linha ORDER BY sort_nome, sort_id), '[]'::jsonb) AS data
    FROM linhas
  ),
  totais_calc AS (
    ...
    FROM jsonb_array_elements((SELECT data FROM ordered)) l
  )
  SELECT data, t INTO _dados, _totais
  FROM ordered, totais_calc;

  -- Hash determinístico (versão da regra + dados ordenados)
  _hash := encode(
    digest(
      convert_to(
        jsonb_build_object(
...
REVOKE EXECUTE ON FUNCTION public.folha_pagamento(uuid, date, date, uuid) FROM anon;
